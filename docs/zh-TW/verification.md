...
| `sudo` PAM 授權 | 🟢 | `fprintd` 僅存在於 `/etc/pam.d/sudo`（2026-08-18 Claim 競賽修復——不在 `common-auth`）；`sudo -k` 後執行 `sudo whoami` 會跳出指紋提示並接受驗證（測試方法見 `fingerprint/README.md` c640-only, Chell 無硬體 N/A）。PAM 堆疊與上述鎖定畫面修復同一 session 驗證 |
