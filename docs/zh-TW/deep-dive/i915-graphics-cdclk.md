...
### 3.1 韌體初始化階段
1. MrChromebox UEFI / Coreboot GOP 韌體在開機通電時，將 eDP 顯示初始化在 **`CDCLK = 337.5 MHz`**（Skylake 預設超低功耗時鐘）。
...
3. 因此，驅動在模組載入當下並未重新計算與調整 CDCLK，**CDCLK 仍舊鎖定在 337.5 MHz**。
4. 當面板進入完整的 $3200 \times 1800$ @ 60Hz 輸出（$361.31\text{ MHz}$）時，頻寬瞬間不足：
   ```text
