...
1. **fallback symlink 缺失**：`alsa-lib` 僅搜尋 `conf.d/${Driver}/${CardLongName}.conf` 與 `conf.d/${Driver}/${Driver}.conf`；發行版僅提供 `Hewlett_Packard-Chell-1.0.conf`，缺少 `AVS I2S SSM4567.conf` / `avs_ssm4567.conf` 等，導致 `alsaucm -c hw:4 dump text` 報 `-2`。
...
```bash
alsaucm -c hw:4 dump text | grep -q "Verb.HiFi" && echo "UCM OK"
alsaucm -c hw:3 dump text | grep -q "Verb.HiFi" && echo "UCM OK"
```
